...
def handler(event: dict, context) -> dict:
    """Приём заявки с сайта: сохраняет в базу данных и пытается отправить на почту"""
    cors_headers = {
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO requests (name, phone, house_type, wall_material, area, color) "
                "VALUES (%s, %s, %s, %s, %s, %s)",
                (name, phone, house_type, wall_material, area, color)
            )
        conn.commit()
    finally:
        conn.close()
    ...
    try:
        ...
        smtp_password = os.environ.get('SMTP_PASSWORD1', '')
